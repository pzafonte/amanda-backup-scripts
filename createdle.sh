...
SSH_ID="amandabackup"
SSH_ID_KEY="/var/amandabackup/.ssh/id_rsa"
...
showusage(){
 ...
 echo "usage:"
 echo "./createdle.sh [OPTIONS] -t zfs-dumptype amanda_config"
 echo "OPTIONS:"
...
ssh -l $SSH_ID -i $SSH_ID_KEY $ZFS_SERVER "/usr/sbin/zfs list -H -o $COL -t filesystem" > $DLE_TEMP


#3. combite zfs and template dle and copy to destination configuration fold
AMANDA_CONF_DLE="$AMANDA_CONF/disklist"
echo "# disklist file created by script createdle.sh " > $AMANDA_CONF_DLE
